...
## What is it?
Probe is a diagnostic tool that runs independently of ByeBlocked. It never modifies or patches it. It does read data that ByeBlocked publishes about its own internal health, which lets Probe cross-reference the two, but Probe's core inspection of Discord's internals works even if ByeBlocked isn't installed.
